...
void Node::Update( double p, double z )
{
	...
	for ( int i = 0; i < this->child.size(); i++ )
	{
		...
		if ( this->child[i]->odds < 1/z )
		{
			...
			if ( tempOdds * text[this->pos+1][this->child[i]->edge] < 1/z )
			{
			...
			{
				tempOdds *= text[this->pos+1][this->child[i]->edge];
			}
			for ( int k = this->pos+2; k < this->child[i]->pos; k++ )
			{
				...
				if ( tempP < 1/z )
				{
					...
					this->child[i]->edge = this->child[i]->edge.substr(0, k);
